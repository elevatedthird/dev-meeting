...
class BlockListingForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($categories as $category) {
      ...
      $weight_delta = round(count($category['blocks']) / 2);

      foreach ($category['blocks'] as $block) {
        $block['category'] = $category_id;
        $form['categories'][$block['id']] = $this->buildBlockRow($block, $weight_delta);
      }
    }
   ...
   * @var array $block
   *   The block.
   * @var int|NULL $weight_delta
   *   Count of blocks in group.
   *
   * @return array
   *   Row with information about block.
   */
  private function buildBlockRow($block, $weight_delta = NULL) {
    $row = [];
    $row['title'] = [
      '#type' => 'markup',
